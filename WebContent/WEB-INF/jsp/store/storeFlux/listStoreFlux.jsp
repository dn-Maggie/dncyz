...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/storeFlux/listStoreFlux.do'/>",/*grid初始化请求数据的远程地址*/
    		...
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "fluxId",hidden : true,key : true,label:"流量ID(主键)",index : "FLUX_ID"},				
				{name : "platformId",label:"平台ID",index : "PLATFORM_ID"},				
				{name : "storeId",label:"店铺ID",index : "STORE_ID"},				
				{name : "storeExposure",label:"曝光量",index : "STORE_EXPOSURE"},				
				{name : "storeVisitorNum",label:"进店人数",index : "STORE_VISITOR_NUM"},				
				{name : "storeOrderNum",label:"下单数",index : "STORE_ORDER_NUM"},				
				{name : "fluxInCategory",label:"品类入口餐厅列表流量",index : "FLUX_IN_CATEGORY"},				
				{name : "fluxInHomepage",label:"首页餐厅列表流量",index : "FLUX_IN_HOMEPAGE"},				
				{name : "fluxInDetaile",label:"订单详情页流量",index : "FLUX_IN_DETAILE"},				
				{name : "fluxInActivity",label:"活动页流量",index : "FLUX_IN_ACTIVITY"},				
				{name : "fluxInCollect",label:"收藏餐厅流量",index : "FLUX_IN_COLLECT"},				
				{name : "fluxInSearch",label:"搜索流量",index : "FLUX_IN_SEARCH"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/storeFlux/toAddStoreFlux.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("fluxId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/storeFlux/toEditStoreFlux.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/storeFlux/toShowStoreFlux.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("fluxId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/storeFlux/deleteStoreFlux.do'/>?key="+ids,
        				cache:false,
