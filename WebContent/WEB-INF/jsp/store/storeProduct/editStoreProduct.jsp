...
$(function() {
	...
	$("#submit").click(function() {
		...
		var paramDatas = {
				...
				storeProductId:$("#edit_storeProductId").val(),
    ...
    <div class="ui-table ui-widget ui-corner-all ui-border" style="margin-top:40px">
		...
		<table class="table" style="height:auto">
				...
				<td class="inputTd">
					<select class="search_select choose_select" name="storeId" id="edit_storeId">
						...
						<c:forEach var="store" items="${store}">
							<option value="${store.storeId}"<c:if test="${storeProduct.storeId==store.storeId}">selected</c:if>> <c:out value="${store.storeName}"></c:out> </option>
			             </c:forEach>
					</select>
				</td>
			...
			<tr>
				...
				<td class="inputTd" colspan="3">
					<form method="post" id="realImageForm" target="realImageIframe" enctype="multipart/form-data" action="<%=request.getContextPath()%>/common/fileUpload">
						...
						<img alt="${storeProduct.productImagePath}"  src="${storeProduct.productImagePath}" width="100%" height="200px" id="productImg">
			...
			<tr>
				...
				<td class="inputTd">
					<select name="productStatus" class="search_select" id="edit_productStatus">
						<c:forEach items="${productStatus}" var="productStatus">
							<option value="${productStatus.dictCode}" 
								<c:if test="${productStatus.dictCode==storeProduct.productStatus}">selected</c:if>>
								<c:out value="${productStatus.dictName}"></c:out> 
							</option>
						</c:forEach>
					</select>
				</td>
			...
			<tr>
				...
				<td class="inputTd">
					<input id="edit_productUnitPrice" name="productUnitPrice" type="number" min="0" class="text number" value="${storeProduct.productUnitPrice}"/>
				</td>
				<td class="inputLabelTd">库存数量：</td>
				<td class="inputTd">
					<input id="edit_productStocks" name="productStocks" type="number" min="0" class="text number" value="${storeProduct.productStocks}"/>
				</td>
			</tr>
			<tr>
