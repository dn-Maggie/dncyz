...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/storePlatformRel/listStorePlatformRel.do'/>",/*grid初始化请求数据的远程地址*/
    		...
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "relId",hidden : true,key : true,label:"关联ID（主键）",index : "REL_ID"},				
				{name : "storeId",label:"店铺ID(主键)",index : "STORE_ID"},				
				{name : "storeName",label:"店铺名称",index : "STORE_NAME"},				
				{name : "platformId",label:"平台ID（主键）",index : "PLATFORM_ID"},				
				{name : "platformCode",label:"平台编码（BD：百度外卖，MT：美团外卖，ELM：饿了么外卖）",index : "PLATFORM_CODE"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/storePlatformRel/toAddStorePlatformRel.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("platformId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/storePlatformRel/toEditStorePlatformRel.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/storePlatformRel/toShowStorePlatformRel.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("platformId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/storePlatformRel/deleteStorePlatformRel.do'/>?key="+ids,
        				cache:false,
