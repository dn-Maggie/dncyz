...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/storeEvaluate/listStoreEvaluate.do'/>",/*grid初始化请求数据的远程地址*/
            datatype: "json",/*数据类型，设置为json数据，默认为json*/
           	sortname:"EVALUATE_ID",
           	sortorder:"asc",
           	pager: '#remote_prowed' /*分页栏id*/,
     		rowList:[10,15,50,100],//每页显示记录数
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "evaluateId",hidden : true,key : true,label:"评价ID(主键)",index : "EVALUATE_ID"},				
				{name : "productId",label:"评价产品ID",index : "PRODUCT_ID"},				
				{name : "channelId",label:"渠道ID",index : "CHANNEL_ID"},				
				{name : "starLevel",label:"星级",index : "STAR_LEVEL"},				
				{name : "evaluateContent",label:"评价内容",index : "EVALUATE_CONTENT"},				
				{name : "replyContent",label:"回复内容",index : "REPLY_CONTENT"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/storeEvaluate/toAddStoreEvaluate.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("evaluateId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/storeEvaluate/toEditStoreEvaluate.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/storeEvaluate/toShowStoreEvaluate.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("evaluateId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/storeEvaluate/deleteStoreEvaluate.do'/>?key="+ids,
        				cache:false,
