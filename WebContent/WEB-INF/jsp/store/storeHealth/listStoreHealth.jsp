...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/storeHealth/listStoreHealth.do'/>",/*grid初始化请求数据的远程地址*/
    		...
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "healthId",hidden : true,key : true,label:"健康ID(主键)",index : "HEALTH_ID"},				
				{name : "storeId",label:"店铺ID",index : "STORE_ID"},				
				{name : "assessDate",label:"评估时间",index : "ASSESS_DATE"},				
				{name : "assessors",label:"评估人",index : "ASSESSORS"},				
				{name : "healthValue",label:"健康值",index : "HEALTH_VALUE"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/storeHealth/toAddStoreHealth.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("healthId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/storeHealth/toEditStoreHealth.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/storeHealth/toShowStoreHealth.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("healthId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/storeHealth/deleteStoreHealth.do'/>?key="+ids,
        				cache:false,
