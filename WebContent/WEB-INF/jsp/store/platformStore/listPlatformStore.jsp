...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/platformStore/listPlatformStore.do'/>",/*grid初始化请求数据的远程地址*/
            datatype: "json",/*数据类型，设置为json数据，默认为json*/
           	sortname:"PLATFORM_STORE_ID",
           	sortorder:"asc",
           	pager: '#remote_prowed' /*分页栏id*/,
     		rowList:[10,15,50,100],//每页显示记录数
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "platformStoreId",hidden : true,key : true,label:"平台店铺ID(主键)",index : "PLATFORM_STORE_ID"},				
				{name : "platformType",label:"平台类别",index : "PLATFORM_TYPE"},				
				{name : "storeId",label:"店铺ID",index : "STORE_ID"},				
				{name : "platformStoreIndex",label:"平台店铺序号",index : "PLATFORM_STORE_INDEX"},				
				{name : "platformStoreName",label:"平台名称",index : "PLATFORM_STORE_NAME"},				
				{name : "platformStoreUsername",label:"平台账号",index : "PLATFORM_STORE_USERNAME"},				
				{name : "platformStorePassword",label:"平台密码，MD5加密",index : "PLATFORM_STORE_PASSWORD"},				
				{name : "platformStoreScore",label:"评分",index : "PLATFORM_STORE_SCORE"},				
				{name : "marketingManager",label:"市场经理",index : "MARKETING_MANAGER"},				
				{name : "marketingManagerTel",label:"市场经理电话",index : "MARKETING_MANAGER_TEL"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/platformStore/toAddPlatformStore.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("platformStoreId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/platformStore/toEditPlatformStore.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/platformStore/toShowPlatformStore.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("platformStoreId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/platformStore/deletePlatformStore.do'/>?key="+ids,
        				cache:false,
