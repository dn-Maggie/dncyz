...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/productList/listProductList.do'/>",/*grid初始化请求数据的远程地址*/
            datatype: "json",/*数据类型，设置为json数据，默认为json*/
           	sortname:"PRODUCT_LIST_ID",
           	sortorder:"asc",
           	pager: '#remote_prowed' /*分页栏id*/,
     		rowList:[10,15,50,100],//每页显示记录数
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "productListId",hidden : true,key : true,label:"订单产品列表ID(主键)",index : "PRODUCT_LIST_ID"},				
				{name : "orderId",label:"订单ID",index : "ORDER_ID"},				
				{name : "productId",label:"产品ID",index : "PRODUCT_ID"},				
				{name : "productCount",label:"数量",index : "PRODUCT_COUNT"},				
				{name : "productUnitPrice",label:"单价",index : "PRODUCT_UNIT_PRICE"},				
				{name : "productCost",label:"金额",index : "PRODUCT_COST"},				
				{name : "productListStatus",label:"订单产品列表状态",index : "PRODUCT_LIST_STATUS"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/productList/toAddProductList.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("productListId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/productList/toEditProductList.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/productList/toShowProductList.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("productListId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/productList/deleteProductList.do'/>?key="+ids,
        				cache:false,
