...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/order/listOrder.do'/>",/*grid初始化请求数据的远程地址*/
            ...
            colModel:[
				...
				{name : "orderPersonTel",label:"订单人电话",index : "ORDER_PERSON_TEL"},				
				{name : "productListId",label:"订单产品列表ID",index : "PRODUCT_LIST_ID"},				
				{name : "deliveryCost",label:"配送费",index : "DELIVERY_COST"},				
				{name : "foodboxCost",label:"餐盒费",index : "FOODBOX_COST"},				
				{name : "privilege",label:"立减优惠",index : "PRIVILEGE"},				
				{name : "deduction",label:"红包抵扣",index : "DEDUCTION"},				
				{name : "orderStatus",label:"订单状态",index : "ORDER_STATUS"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/order/toAddOrder.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("orderId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/order/toEditOrder.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/order/toShowOrder.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("orderId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/order/deleteOrder.do'/>?key="+ids,
        				cache:false,
