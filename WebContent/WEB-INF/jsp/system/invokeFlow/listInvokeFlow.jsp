...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/invokeFlow/listInvokeFlow.do'/>",/*grid初始化请求数据的远程地址*/
    		...
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "flowId",hidden : true,key : true,label:"流程ID（主键）",index : "FLOW_ID"},				
				{name : "platformId",label:"平台ID（主键）",index : "PLATFORM_ID"},				
				{name : "opId",label:"功能ID（主键）",index : "OP_ID"},				
				{name : "url",label:"调用外卖平台接口url",index : "URL"},				
				{name : "invokeIndex",label:"接口调用顺序",index : "INVOKE_INDEX"},				
				{name : "remark",label:"平台描述",index : "REMARK"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/invokeFlow/toAddInvokeFlow.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("flowId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/invokeFlow/toEditInvokeFlow.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/invokeFlow/toShowInvokeFlow.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("flowId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/invokeFlow/deleteInvokeFlow.do'/>?key="+ids,
        				cache:false,
