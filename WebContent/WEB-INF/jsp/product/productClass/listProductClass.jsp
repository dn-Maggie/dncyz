...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/productClass/listProductClass.do'/>",/*grid初始化请求数据的远程地址*/
            datatype: "json",/*数据类型，设置为json数据，默认为json*/
           	sortname:"PRODUCT_CLASS_ID",
           	sortorder:"asc",
           	pager: '#remote_prowed' /*分页栏id*/,
     		rowList:[10,15,50,100],//每页显示记录数
    		rowNum:10,//默认显示15条
            colModel:[
				{name : "productClassId",hidden : true,key : true,label:"产品分类ID(主键)",index : "PRODUCT_CLASS_ID"},				
				{name : "productClassName",label:"产品分类名称",index : "PRODUCT_CLASS_NAME"}				
           	],
	...
	var show_iframe_dialog;
  	...
  	function add(){
  	//xin zeng iframe 弹出框
		var url="<m:url value='/productClass/toAddProductClass.do'/>";
    ...
    function edit(){
		var key = ICSS.utils.getSelectRowData("productClassId");
		if(key.indexOf(",")>-1||key==""){
			showMessage("请选择一条数据！");
			return ;
		}
		var url="<m:url value='/productClass/toEditProductClass.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/productClass/toShowProductClass.do'/>?key="+key;
    ...
    */
    ...
    function batchDelete(){
    	var ids = ICSS.utils.getSelectRowData("productClassId");
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/productClass/deleteProductClass.do'/>?key="+ids,
        				cache:false,
